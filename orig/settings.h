...
const int g_nEvent = 2e5; // number of events per realization (and ExI in bExSpread)
...
const int g_nZ = 75; // proton number
const int g_nAMass = 187; // proton + neutron number
const int g_nDisLvlMax = 20; // only trust level scheme to here, sets ECrit
const bool g_bIsEvenA = !(g_nAMass % 2);
const int g_nDisLvlGamMax = g_nDisLvlMax; // max gammas in for a discrete lvl
...
int g_nConEBin;
#endif // bForceBinNum
const int g_nConSpbMax = 20+1; // constructed # spin(s) (#bins = +1); can choose small for light ion rxn
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  0.505; // MeV
  const double g_dE0   =  -0.635958; // MeV
  ...
  double g_dE1Usr = -0.490; // shift for Ex in spin-cut -- in Oslo different from g_dE0/g_dE1! 
  #endif
...
const double g_dExIMax = 6.0; // MeV; constructed lvl scheme built up to this
// dont exceed with init excitations - gaus might sample higher than expected
const double g_adExIMean[] = {3.0, 4.0, 5.0}; // MeV
...
const double g_dExIMax = 6.4; // MeV; above max population energy
const char popFile[] = "../misc/talys/nld_pop.txt"; // made from TALYS "outpopulation y"
// make sure to match # of discrete bins. See ReadPopFile() bins + maxlevelstar + 1 = g_nExPopI
const int g_nExPopI = 66-2; // bins 0-70; bins + maxlevelstar + 1 = g_nExPopI
const int g_nSpPopIBin = 21; // spins 0-20
